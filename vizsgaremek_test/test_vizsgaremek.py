...
class TestConduit(object):
    ...
    def test_regRight(self):
        ...
        self.browser.find_element(By.XPATH, '//input[@placeholder="Username"]').send_keys(
            regUserDict["userRight"]["name"])
        self.browser.find_element(By.XPATH, '//input[@placeholder="Email"]').send_keys(
            regUserDict["userRight"]["email"])
        self.browser.find_element(By.XPATH, '//input[@placeholder="Password"]').send_keys(
            regUserDict["userRight"]["password"])
        self.browser.find_element(By.XPATH, '//button[@class="btn btn-lg btn-primary pull-xs-right"]').click()
        WebDriverWait(self.browser, varj).until(
            EC.presence_of_element_located((By.XPATH, '//button[@class="swal-button swal-button--confirm"]'))).click()
        assert WebDriverWait(self.browser, varj2).until(
            EC.presence_of_all_elements_located((By.XPATH, '//a[@class="nav-link"]')))[2].text == \
               regUserDict["userRight"][
                   "name"]
    ...
    def test_regBadEmail(self):
        ...
        self.browser.find_element(By.XPATH, '//input[@placeholder="Password"]').send_keys(
            regUserDict["userBad"]["password"])
        self.browser.find_element(By.XPATH, '//button[@class="btn btn-lg btn-primary pull-xs-right"]').click()
        time.sleep(varj)
        assert WebDriverWait(self.browser, varj2).until(EC.presence_of_element_located(
            (By.XPATH, '//div[@class="swal-text"]'))).text == "Email must be a valid email."
    ...
    def test_loginRight(self):
        ...
        assert WebDriverWait(self.browser, varj).until(
            EC.presence_of_element_located(
                (By.XPATH, '//a[@class="nav-link router-link-exact-active active"]'))).is_displayed()
    ...
    def test_logOut(self):
        login(self.browser)
        WebDriverWait(self.browser, varj).until(
            EC.presence_of_element_located((By.XPATH, '//a[@active-class="active"]'))).click()
        assert WebDriverWait(self.browser, varj).until(
            EC.presence_of_element_located((By.XPATH, '//a[@href="#/login"]'))).is_displayed()
    ...
    def test_loginEmpty(self):
        ...
        time.sleep(varj)
        assert WebDriverWait(self.browser, varj).until(
            EC.presence_of_element_located((By.XPATH, '//div[@class="swal-text"]'))).text == "Email field required."
    ...
    def test_loginBadPass(self):
        ...
        time.sleep(varj)
        assert WebDriverWait(self.browser, varj).until(
            EC.presence_of_element_located((By.XPATH, '//div[@class="swal-text"]'))).text == "Invalid user credentials."

    # ------------------------------------------------------------------------------------------------------------------
    # ATC09 - Global Feed oldalainak lapozása
    def test_pageNumber(self):
        login(self.browser)
        for page in WebDriverWait(self.browser, varj).until(
                EC.presence_of_all_elements_located((By.XPATH, '//a[@class="page-link"]'))):
            page.click()
            actual_page = WebDriverWait(self.browser, varj).until(
    ...
    def test_tags(self):
        login(self.browser)
        WebDriverWait(self.browser, varj).until(EC.presence_of_element_located(
            (By.XPATH, '//div[@class="sidebar"]/div[@class="tag-list"]/a[@href="#/tag/dolor"]'))).click()
        assert len(WebDriverWait(self.browser, 5).until(
            EC.presence_of_all_elements_located((By.XPATH, '//a[@class="preview-link"]/h1')))) != 0
    ...
    def test_articleCreate(self):
        ...
        self.browser.find_element(By.XPATH, '//textarea[@placeholder="Write your article (in markdown)"]').send_keys(
            articleDict["markDown"])
        self.browser.find_element(By.XPATH, '//input[@placeholder="Enter tags"]').send_keys(articleDict["tags"])
        self.browser.find_element(By.XPATH, '//button[@type="submit"]').click()
        assert WebDriverWait(self.browser, varj).until(EC.presence_of_element_located((By.XPATH, '//h1'))).text == \
               articleDict["title"]
    ...
    def test_commentCreate(self):
        ...
        create_article(self.browser)
        WebDriverWait(self.browser, varj).until(
            EC.presence_of_element_located((By.XPATH, '//textarea[@placeholder="Write a comment..."]'))).send_keys(
            comment)
        self.browser.find_element(By.XPATH, '//button[@class="btn btn-sm btn-primary"]').click()
        assert WebDriverWait(self.browser, varj).until(
            EC.presence_of_all_elements_located((By.XPATH, '//div[@class="card"]')))[0].is_displayed()
    ...
    def test_commentDelete(self):
        login(self.browser)
        WebDriverWait(self.browser, varj).until(
            EC.presence_of_element_located((By.XPATH, '//a[@class="preview-link"][1]'))).click()
        time.sleep(varj2)
        create_comment(self.browser)
        WebDriverWait(self.browser, varj).until(
            EC.presence_of_element_located((By.XPATH, '//i[@class="ion-trash-a"]'))).click()
        assert len(WebDriverWait(self.browser, varj).until(
    ...
    def test_importCsv(self):
        login(self.browser)
        with open('dataImp.csv', 'r') as file:
            csv_reader = csv.reader(file, delimiter=',')
            for row in csv_reader:
                create_article_data(self.browser, row[0], row[1], row[2], row[3])
                assert WebDriverWait(self.browser, varj).until(
                    EC.presence_of_element_located((By.XPATH, '//h1'))).text == row[0]

    # ------------------------------------------------------------------------------------------------------------------
    # ATC16 - Címkék lementése csv adatfájlba
    def test_tag2Csv(self):
        login(self.browser)
        tag_list = WebDriverWait(self.browser, varj).until(EC.presence_of_all_elements_located(
            (By.XPATH, '//div[@class="sidebar"]/div/a[@class="tag-pill tag-default"]')))
        with open('listTag.csv', 'w') as file:
    ...
    def test_nameModifie(self):
        login(self.browser)
        WebDriverWait(self.browser, varj).until(
            EC.presence_of_all_elements_located((By.XPATH, '//a[@class="nav-link"]')))[1].click()
        WebDriverWait(self.browser, varj).until(
            EC.presence_of_element_located((By.XPATH, '//input[@placeholder="Your username"]'))).clear()
        WebDriverWait(self.browser, varj).until(
            EC.presence_of_element_located((By.XPATH, '//input[@placeholder="Your username"]'))).send_keys(newName)
        WebDriverWait(self.browser, varj).until(EC.presence_of_element_located(
            (By.XPATH, '//button[@class="btn btn-lg btn-primary pull-xs-right"]'))).click()
        WebDriverWait(self.browser, varj).until(
            EC.presence_of_element_located((By.XPATH, '//button[@class="swal-button swal-button--confirm"]'))).click()
        assert WebDriverWait(self.browser, varj).until(
            EC.presence_of_all_elements_located((By.XPATH, '//a[@class="nav-link"]')))[2].text == newName
